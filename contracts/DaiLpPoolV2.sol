...
contract DaiLpPoolV2 is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    function _withdrawMphVested(uint256 depositId) internal {
        ...
        Vest memory vest = mphVesting.accountVestList(
            address(this),
            _vestingIdx
        );
        require(
        ...
        if (vested > 0) {
            ...
            uint256 takeBackAmount = vested.mul(mphTakeBackMultiplier).div(
                1e18
            );
        ...
        returns (uint256)
    {
        ...
        (uint256 daiDepositId, uint256 maturationTimestamp) = _depositDai(
            daiAmount
        );
    ...
    function _withdrawDai(uint256 depositId, uint256 fundingId) internal {
        ...
        uint256 daiBalance = dai.balanceOf(address(this));
        uint256 daiReward = daiBalance.sub(daiOldBalance).sub(
            depositInfo.daiAmount
        );
